...
private:
    ...
    bool compatible(const LockRequestQueue &request_queue, txn_id_t txn_id,
                    LockMode lock_mode) const;
    bool is_stronger_or_equal(LockMode held, LockMode requested) const;
    void recompute_group_lock_mode(LockRequestQueue &request_queue);
