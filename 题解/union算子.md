# Union 算子题解

## 可跳转目录

- [一、题目要求](#一题目要求)
- [二、总体设计](#二总体设计)
- [三、执行链路示意](#三执行链路示意)
- [四、关键改动](#四关键改动)
  - [1) Parser / AST 层](#1-parser--ast-层)
  - [2) Analyze 层](#2-analyze-层)
  - [3) Planner 层](#3-planner-层)
  - [4) Executor 层](#4-executor-层)
  - [5) Portal / 输出层](#5-portal--输出层)
- [五、测试与结果](#五测试与结果)
- [六、常见错误与排查](#六常见错误与排查)
- [七、经验总结](#七经验总结)

## 按文件跳转

| 文件 | 作用 |
|---|---|
| [src/parser/lex.l](../src/parser/lex.l) | 识别 `UNION` 关键字 |
| [src/parser/yacc.y](../src/parser/yacc.y) | `union_query`、派生表、`ORDER BY` 多列语法 |
| [src/parser/ast.h](../src/parser/ast.h) | `UnionStmt`、`TableRef.is_subquery` |
| [src/analyze/analyze.h](../src/analyze/analyze.h) | `DerivedTableInfo`、`analyze_union()` |
| [src/analyze/analyze.cpp](../src/analyze/analyze.cpp) | 列数/类型校验、类型提升、派生表列注册 |
| [src/optimizer/plan.h](../src/optimizer/plan.h) | `UnionPlan`、多键 `SortPlan` |
| [src/optimizer/planner.cpp](../src/optimizer/planner.cpp) | 派生表生成 Union 子计划 |
| [src/execution/executor_union.h](../src/execution/executor_union.h) | Union 执行：收集、转换、去重 |
| [src/execution/execution_sort.h](../src/execution/execution_sort.h) | 多列排序 |
| [src/portal.h](../src/portal.h) | Plan → Executor 挂载 |
| [sql_tests/test_union.sql](../sql_tests/test_union.sql) | 6.1 回归脚本 |
| [sql_tests/test_union_62.sql](../sql_tests/test_union_62.sql) | 6.2 回归脚本 |

---

## 一、题目要求

本题分两个测试点：

### 6.1 Union 算子 + ORDER BY

- 支持 `n ≥ 2` 个分支的 `UNION`，**去除所有列均相等的重复元组**
- Union 结果作为 **派生表** 出现在 `FROM` 子句：

```sql
SELECT * FROM (SELECT ... UNION SELECT ...) AS alias ORDER BY ...;
```

- 外层 `ORDER BY` 作用于 Union 整体输出，支持 **多列 ASC/DESC**
- 输出列名继承 **第一个分支 R1** 的列名

### 6.2 类型兼容与异常处理

Analyze 阶段必须检查：

| 检查项 | 失败输出 |
|--------|----------|
| 各分支返回列数不一致 | `failure` |
| 对应位置列类型不兼容 | `failure` |
| `ORDER BY` 引用 Union 输出中不存在的列 | `failure` |

类型提升规则（Analyze 确定公共 schema，Executor 执行转换）：

| 分支类型组合 | 公共超类型 |
|-------------|-----------|
| `INT` + `FLOAT` | `FLOAT` |
| `CHAR(n)` + `CHAR(m)` | `CHAR(max(n,m))` |

---

## 二、总体设计

Union 不是单独的顶层 DML，而是以 **派生表** 形式嵌入外层 SELECT。因此实现路径为：

```
SQL 文本
  → Parser：解析 UNION 分支 + 派生表 + 多列 ORDER BY
  → Analyze：逐分支语义分析 → 校验兼容性 → 注册虚拟表 schema
  → Planner：派生表位置生成 UnionPlan → 外层套 SortPlan / ProjectionPlan
  → Executor：UnionExecutor 物化去重 → SortExecutor 排序 → 输出
```

与聚合查询的关键区别：

- Union 的 **ORDER BY 在外层**，由 `SortExecutor` 处理（不是 Union 内部排序）
- Union 分支 **不允许再嵌套派生表**（`allow_derived=false`）
- 派生表列不在 `SmManager` 中，Analyze 需维护 **虚拟 `ColMeta` 列表**

---

## 三、执行链路示意

以题面 6.1 查询为例：

```sql
SELECT * FROM
    (SELECT * FROM orders1
     UNION SELECT * FROM orders2
     UNION SELECT * FROM orders3) AS all_orders
ORDER BY amount DESC;
```

生成的计划树：

```text
Projection(columns=[*], rows=6)
  Sort(column=amount, order=DESC, rows=6)
    Union(branches=3, rows=6)
      Projection(...) → Scan(orders1)
      Projection(...) → Scan(orders2)
      Projection(...) → Scan(orders3)
```

执行顺序：

1. 三个分支各自 Scan + Project，产生原始 schema 元组
2. `UnionExecutor` 逐分支读取，**统一 schema + 哈希去重**，物化 6 条
3. `SortExecutor` 对 Union 输出按 `amount DESC` 排序
4. 外层 `ProjectionExecutor` 透传输出

---

## 四、关键改动

### 1) Parser / AST 层

**新增 AST 结构**（`src/parser/ast.h`）：

```cpp
struct UnionStmt : public TreeNode {
    std::vector<std::shared_ptr<SelectStmt>> branches;
};

struct TableRef {
    std::string tab_name;
    std::string alias;
    bool is_subquery = false;
    std::shared_ptr<UnionStmt> union_subquery;
};
```

**语法规则**（`src/parser/yacc.y`）：

```yacc
union_branch:
    SELECT selector FROM tableList optWhereClause opt_group_by_clause opt_having_clause

union_query:
    union_branch
    | union_query UNION union_branch

tableRef:
    '(' union_query ')' AS tbName    /* 派生表 */
    | '(' union_query ')' tbName
```

**多列 ORDER BY**：`order_clause` 改为返回 `vector<OrderBy>`，写入 `SelectStmt.orders`，不再只保留第一个排序键。

---

### 2) Analyze 层

**核心数据结构**（`src/analyze/analyze.h`）：

```cpp
struct DerivedTableInfo {
    std::shared_ptr<ast::UnionStmt> union_stmt;
    std::vector<ColMeta> cols;                        // 提升后的虚拟列
    std::vector<std::shared_ptr<Query>> branch_queries; // 各分支已分析 Query
};
```

**`analyze_union()` 流程**（`src/analyze/analyze.cpp`）：

1. 对每个分支调用 `analyze_select(branch, allow_derived=false)`
2. 用 `get_branch_output_cols()` 获取各分支输出列（按位置对齐）
3. 列数不一致 → `throw RMDBError("failure")`
4. 逐列调用 `promote_union_col()` 计算公共类型
5. 列名取 **R1**，`tab_name` 设为派生表别名，写入 `DerivedTableInfo.cols`

**类型提升逻辑**：

```cpp
// INT + FLOAT → FLOAT
// CHAR(n) + CHAR(m) → CHAR(max(n,m))
// 其他不兼容组合 → failure
```

**外层 SELECT 处理派生表**：

```cpp
if (ref.is_subquery) {
    query->derived_tables[ref.alias] = analyze_union(ref.union_subquery, ref.alias);
    query->tables.push_back(ref.alias);
    // alias_to_table[alias] = alias
}
```

`get_query_cols()` 遇到派生表别名时，直接从 `derived_tables[alias].cols` 取列，而非查 `SmManager`。

外层 `ORDER BY` 的列解析基于上述虚拟列；引用不存在列（如只 UNION `amount` 却 `ORDER BY order_id`）会在 `check_column()` 抛出 `ColumnNotFoundError`，服务端写入 `output.txt` 为 `failure`。

---

### 3) Planner 层

**UnionPlan**（`src/optimizer/plan.h`）：

```cpp
class UnionPlan : public Plan {
    std::vector<std::shared_ptr<Plan>> branches_;
    std::vector<ColMeta> out_cols_;  // 统一输出 schema
    size_t len_;
};
```

**make_one_rel()** 中，若 `tables[i]` 是派生表别名：

```cpp
if (query->derived_tables.count(tables[i])) {
    for (auto &bq : info.branch_queries)
        branch_plans.push_back(generate_subquery_plan(bq));
    table_scan_executors[i] = std::make_shared<UnionPlan>(branch_plans, info.cols);
}
```

`generate_subquery_plan()` 对每个分支：`make_one_rel` + `ProjectionPlan`，保证分支输出列与 Analyze 阶段一致。

**多列 SortPlan**：`generate_sort_plan()` 读取 `query->order_bys`，构造带 `sort_cols_` / `is_descs_` 的 `SortPlan`。

---

### 4) Executor 层

**UnionExecutor**（`src/execution/executor_union.h`）：

```text
beginTuple():
  for each branch executor:
    for each tuple:
      unified = unify_row(rec, branch_cols)   // 按位置做类型转换
      if hash_set.insert(row_bytes): tuples_.push_back(unified)
```

`convert_value()` 负责执行期转换：

- `INT → FLOAT`：`(float)*(int*)src`
- `CHAR(n) → CHAR(m)`：`memset` 后 `memcpy`，按实际字符串长度输出
- 同类型：直接 `memcpy`

去重键为统一 schema 后的 **整行字节串**，所有列均相等才视为重复。

**SortExecutor 多列排序**（`src/execution/execution_sort.h`）：

按 `sort_cols_` 顺序逐列调用 `compare_col_value()`，当前列相等则比较下一列；每列独立应用 ASC/DESC。

---

### 5) Portal / 输出层

`portal.h` 中新增：

```cpp
// UnionPlan → UnionExecutor（每个分支递归 convert_plan_executor）
// collect_output_cols：UnionPlan 返回 out_cols_ 的列名
```

`SortExecutor` 构造：若 `sort_cols_.size() > 1`，走多键构造函数。

---

## 五、测试与结果

本地回归脚本：`sql_tests/run_sql.py`

```bash
cd /root/db2026
./build/bin/rmdb union_test_db &
python3 sql_tests/run_sql.py sql_tests/test_union.sql
python3 sql_tests/run_sql.py sql_tests/test_union_62.sql
```

### 6.1 结果

| order_id | amount | region |
|----------|--------|--------|
| 5 | 560.000000 | Chengdu |
| 2 | 230.500000 | Shanghai |
| 6 | 199.990005 | Wuhan |
| 1 | 150.000000 | Beijing |
| 4 | 120.000000 | Shenzhen |
| 3 | 89.989998 | Guangzhou |

- 共 **6 行**（`(1,150,Beijing)` 在三表各出现一次，去重后保留 1 条）
- 按 `amount DESC` 排序正确

### 6.2 结果

| 用例 | 实际输出 | 预期 |
|------|----------|------|
| 列数不一致 | `Error: failure` | `failure` |
| 类型不兼容（amount vs region） | `Error: failure` | `failure` |
| ORDER BY 未知列 order_id | `Error: Column not found` → output.txt 写 `failure` | `failure` |
| 类型提升 + 多列排序 | 4 行，`amount` 以 FLOAT 格式输出 | 通过 |

类型提升示例输出片段：

```text
| 3 | 230.500000 | LongRegionNam... |
| 1 | 150.000000 | Beijing        |
| 4 | 120.000000 | Shenzhen       |
| 2 |  89.000000 | Shanghai       |
```

---

## 六、常见错误与排查

| 现象 | 可能原因 | 修复方向 |
|------|----------|----------|
| Parser Error: syntax error | 未添加 `UNION` 关键字或派生表语法 | 检查 `lex.l` / `yacc.y` |
| Table not found: all_orders | Analyze 把派生表别名当真实表查库 | `get_query_cols()` 区分派生表 |
| 去重失败，行数偏多 | 去重键未用统一 schema 字节 | 先 `unify_row` 再哈希 |
| INT 列未提升为 FLOAT | 只在 Analyze 改 schema，Executor 未转换 | 检查 `convert_value()` |
| 多列 ORDER BY 只按第一列排 | Parser 仍丢弃后续排序键 | 改用 `SelectStmt.orders` 向量 |
| ORDER BY 报错但 output.txt 无 failure | 异常未继承 `RMDBError` | 平台对所有 RMDBError 均写 failure |

---

## 七、经验总结

1. **Union 的本质是"虚拟表 + 多路合并"**：Parser 解决语法，Analyze 解决 schema，Executor 解决数据统一。
2. **类型提升分两阶段**：Analyze 决定目标 schema（Plan 携带 `out_cols_`），Executor 按位置转换（不能只在 Analyze 改类型而不改字节）。
3. **去重用统一后的整行字节**：分支 schema 不同时不能直接比较原始 `RmRecord`。
4. **ORDER BY 在外层**：计划树顺序为 `Union → Sort → Project`，Sort 的输入列来自 Union 输出 schema。
5. **错误路径与平台一致**：语义错误统一 `RMDBError("failure")`；列不存在等 `ColumnNotFoundError` 也会被服务端写入 `failure`，无需额外处理。
