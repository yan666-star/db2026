...
基础的 Nested Loop Join 采用双层循环嵌套的逻辑。对于外表（驱动表）$R$ 中的每一行元组 $r$，算子都会触发内表（被驱动表）$S$ 的一次全量扫描。
...
- **原理**：通过笛卡尔积式的遍历，对每一对 $(r,s)$ 元组应用连接谓词。
- **复杂度**：若表规模分别为 $N$ 和 $M$，则时间复杂度为 $O(N \times M)$。
...
- **复杂度**：时间复杂度优化为 $O(N \times \log M)$。对于大数据量，这种优化将带来量级的性能提升。
...
[JOIN table3 ON table1.join_column = table3.join_column
...
嵌套循环连接的执行形态固定为左深树：`表1 JOIN 表2 JOIN 表3` 按 `((表1 JOIN 表2) JOIN 表3)` 执行。第一个表是初始外表/驱动表；每遇到一个新的 `JOIN 右表 ON ...`，当前已经得到的左侧 Join 结果作为外表/驱动输入，右表作为内表/被驱动输入。

例如，`FROM a JOIN b ON a.id=b.a_id JOIN c ON a.id=c.a_id` 的执行树为 `(a JOIN b) JOIN c`，其中第二层 Join 的外表是 `a JOIN b` 的结果，内表是 `c`。
...
`EXPLAIN ANALYZE` 的输出为文本形式的执行计划与运行时统计，不输出该查询本身的结果集。每个计划节点的 `rows=N` 表示该节点本次执行时涉及的总行数。

- 在 NLJ 中，右表作为内表会被左表重复扫描，因此右侧 SeqScan 的 `rows` 可以累计到“左表行数 × 右表行数”。
- 在 INLJ 中，使用右表唯一索引进行点查时，右侧 IndexScan 的 `rows` 只统计命中的行数。

本题中 `EXPLAIN ANALYZE` 的算子输出规范如下：
...
| Project 节点 | `Project(columns=[表名1.列名1,表名2.列名2,...], rows=行数)` | `SELECT` 语句根节点一定是 Project 节点。列名采用 `表名.列名` 形式并按字典序输出；保留全部列时输出 `Project(columns=[*], rows=行数)`。 |
...
$$
\frac{t_{\mathrm{INLJ}}}{t_{\mathrm{NLJ}}} < 0.5
$$
...
- `departments` 表（左表/驱动表）有 5 行，`dept_id` 为部门编号。
- `employees` 表（右表/被驱动表）有 5 行，每个员工属于唯一部门，`dept_id` 值各不相同。
- Join 结果为一一对应，共 5 行。
...
- Scan 的 `type` 变为 `IndexScan`，且使用右表连接列 `dept_id` 上的索引。
