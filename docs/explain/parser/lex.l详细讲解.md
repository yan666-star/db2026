...
关键结论：`caseless` 是"关键字大小写不敏感"的根源，但**标识符 `[a-zA-Z]` 本身大小写敏感**，因此表名 `A` 和 `a` 是两个不同标识符。这与 SQL 关键字不同，做题时不要假设表名自动忽略大小写。
...
**新增关键字的铁律**：关键字规则必须放在 `{identifier}` 规则**之前**。因为 flex 是"最长匹配优先，同长时按定义顺序取第一条"。如果 `"SEMI"` 写在 `{identifier}` 之后，`semi` 会先被标识符规则匹配（都是 4 字符同长，谁在前谁赢），SEMI 就永远匹配不到。
...
Flex 每匹配一个 token、执行动作**之前**，会先执行这段宏。它遍历 `yytext` 更新 `yylloc`（当前 token 的行列）。这样报错时能报出行号。做题通常不碰它。
...
## 七、新增关键字的标准改法（以 SEMI 为例）

第 1 步：在 JOIN 附近、且**必须在 `{identifier}` 之前**，加行首规则：

```lex
"SEMI" { return SEMI; }
```

**规则必须从行首写，不能缩进**。flex 中行首的规则才被当作"正则模式"，缩进的会被当作动作代码（C 代码），直接编译报错。

第 2 步：在 yacc.y 里 `%token SEMI` 声明同名 token。

第 3 步：改完必须**重新跑 cmake 配置 + 编译**，因为 `src/parser/CMakeLists.txt` 在配置阶段会删除旧的 `lex.yy.cpp` 再从 lex.l 生成。只 `make` 旧目标不会重新生成词法器。
...
4. **改 lex.l 必须重跑 cmake**，不能只编译旧目标，也不能手改生成的 `lex.yy.cpp`。
5. **`caseless` 只管关键字**，表名/列名（标识符）大小写敏感。
6. **lex 声明 token 但 yacc 没声明** → 编译失败；**yacc 声明但 lex 不返回** → 语法永远匹配不到。两边必须闭环。
7. 新关键字至少要测：合法语句、缺关键字、非法组合、大小写混写四种情况。
