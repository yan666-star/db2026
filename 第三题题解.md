...
**批量扫描（对齐 RMDB2025，commit `35e6077`）**：

1. `beginTuple` 用 `IxScan` 收集范围内全部 RID，按 `page_no` 分组  
2. 每页调用 `batch_get_records` 一次取多行  
3. `eval_conditions` 过滤剩余 WHERE  

满足测点 2/3 的性能检测（真正走索引，非全表 SeqScan）。

### 3.5 DML 索引维护（`executor_*.h`）

| 执行器 | 行为 |
|--------|------|
| `InsertExecutor` | **先**对所有索引 `get_value` 查重，违规则 `failure`；**再** `insert_record`；**最后**各索引 `insert_entry`（顺序必须与 RMDB2025 一致） |
...
**平台测点 5（唯一性）常见失败原因**：旧版 `InsertExecutor` 先 `insert_record` 再查索引，违反唯一约束时仍会写入表，导致 `output.txt` 多出一行且后续查询结果错误。修复后违例语句只向 `output.txt` 追加 `failure\n`，表与索引均不变。
...
3. **字符串键**：`build_*_key` 优先使用 `cond.rhs_val.raw`（`init_raw` 后的定长 `CHAR(n)` 字节），避免范围扫描键与索引键不一致。
...
| `IndexScanExecutor` | 批量 RID + `batch_get_records`（已对齐） |
| `InsertExecutor` | 先查重再写表（已修复测点 5） |
